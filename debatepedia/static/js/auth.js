...
function showAuth(mode){
  ...
  document.body.insertAdjacentHTML('beforeend',`<div class="auth-overlay" id="authOverlay"><div class="auth-card"><button class="auth-close" id="authClose">×</button><h2>${login?'Log in':'Create account'}</h2>${login?'':'<p class="auth-hint">Create an account to submit contributions.</p>'}<input id="auth-identifier" placeholder="${login?'Username or email':'Username'}">${login?'':'<input id="auth-email" type="email" placeholder="Email">'}<input id="auth-password" type="password" placeholder="Password"><div id="authError" class="auth-error"></div><button class="btn btn-primary" id="authSubmit">${login?'Log in':'Create account'}</button><button class="auth-switch" id="authSwitch">${login?'Need an account? Create one':'Already have an account? Log in'}</button></div></div>`);
